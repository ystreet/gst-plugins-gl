...
static gchar *
slider_fps_cb (GtkScale * scale, gdouble value, GstElement * pipeline)
{
  //change the video frame rate dynamically
  return g_strdup_printf ("video framerate: %0.*g",
...
main (gint argc, gchar * argv[])
{
  ...
  gtk_window_set_title (GTK_WINDOW (window),
      "glimagesink implement the gstxoverlay interface");
  ...
  geometry.max_width = -1;
  geometry.max_height = -1;
  gtk_window_set_geometry_hints (GTK_WINDOW (window_control), window_control,
  ...
  GtkWidget *button_state_paused =
      gtk_button_new_with_label ("GST_STATE_PAUSED");
  g_signal_connect (G_OBJECT (button_state_paused), "clicked",
      G_CALLBACK (button_state_paused_cb), pipeline);
  gtk_table_attach_defaults (GTK_TABLE (table), button_state_paused, 0, 1, 2,
      3);
  gtk_widget_show (button_state_paused);

  //control state playing
  GtkWidget *button_state_playing =
  ...
  gtk_table_attach_defaults (GTK_TABLE (table), button_state_playing, 0, 1, 3,
      4);
  ...
  GstElement *videosrc =
      gst_element_factory_make ("videotestsrc", "videotestsrc");
  ...
  GstElement *glfilterlaplacian =
      gst_element_factory_make ("glfilterlaplacian", "glfilterlaplacian");
  GstElement *videosink =
      gst_element_factory_make ("glimagesink", "glimagesink");
  ...
  gst_bin_add_many (GST_BIN (pipeline), videosrc, glupload, glfiltercube,
      glfilterlaplacian, videosink, NULL);
  ...
  if (!gst_element_link_many (glupload, glfiltercube, glfilterlaplacian,
          videosink, NULL)) {
