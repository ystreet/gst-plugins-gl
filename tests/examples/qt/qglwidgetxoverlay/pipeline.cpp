...
Pipeline::create ()
{
  ...
  g_object_set (G_OBJECT (m_glimagesink), "client-reshape-callback",
      reshapeCallback, NULL);
  g_object_set (G_OBJECT (m_glimagesink), "client-draw-callback", drawCallback,
      NULL);

  gst_bin_add_many (GST_BIN (m_pipeline), videosrc, decodebin, m_glupload,
      m_glimagesink, NULL);

  gboolean link_ok =
      gst_element_link_filtered (m_glupload, m_glimagesink, outcaps);
...
void
     Pipeline::doExpose () const const const
     {
...
void
Pipeline::reshapeCallback (uint width, uint height)
{
