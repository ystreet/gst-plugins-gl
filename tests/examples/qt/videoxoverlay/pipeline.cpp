...
Pipeline::create ()
{
  ...
  g_object_set (G_OBJECT (videosrc), "location",
      m_videoLocation.toAscii ().data (), NULL);

  gst_bin_add_many (GST_BIN (m_pipeline), videosrc, decodebin, m_glimagesink,
      NULL);
...
Pipeline::seek ()
{
  if (gst_element_seek (m_pipeline, 1.0, GST_FORMAT_TIME, GST_SEEK_FLAG_FLUSH,
          GST_SEEK_TYPE_SET, 2 * GST_SECOND, GST_SEEK_TYPE_SET, 20 * GST_SECOND)
      )
...
void
Pipeline::setState (GstState state)
{
...
void
     Pipeline::unconfigure () const const const
     {
...
//redraw the current frame in the drawable
     void Pipeline::doExpose () const const const
     {
...
gboolean Pipeline::bus_call (GstBus * bus, GstMessage * msg, Pipeline * p)
{
  ...
  switch (GST_MESSAGE_TYPE (msg)) {
    ...
    case GST_MESSAGE_STATE_CHANGED:
    {
      ...
      gst_message_parse_state_changed (msg, &oldState, &currentState,
          &pendingState);
      if (oldState == GST_STATE_READY && currentState == GST_STATE_PAUSED
          && pendingState == GST_STATE_PLAYING)
        p->seek ();
      break;
    }
    default:
...
void
Pipeline::cb_new_pad (GstElement * decodebin, GstPad * pad, gboolean last,
...
void
Pipeline::cb_video_size (GstPad * pad, GParamSpec * pspec, Pipeline * p)
{
...
GstBusSyncReply
    Pipeline::create_window (GstBus * bus, GstMessage * message,
    const Pipeline * p)
{
