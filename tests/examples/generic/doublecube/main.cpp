...
static void
identityCallback (GstElement * src, GstBuffer * buffer,
...
gboolean
drawCallback (GLuint texture, GLuint width, GLuint height)
{
  ...
  if ((current_time.tv_sec - last_sec) >= 1) {
    std::cout << "GRAPHIC FPS of the scene which contains the custom cube) = "
        << nbFrames << std::endl;
    nbFrames = 0;
...
static void
cb_new_pad (GstElement * decodebin, GstPad * pad, gboolean last,
    GstElement * identity)
{
...
main (gint argc, gchar * argv[])
{
  ...
  GstElement *glimagesink0 =
      gst_element_factory_make ("glimagesink", "glimagesink0");

  GstElement *queue1 = gst_element_factory_make ("queue", "queue1");
  GstElement *glupload1 = gst_element_factory_make ("glupload", "glupload1");
  GstElement *glfiltercube =
      gst_element_factory_make ("glfiltercube", "glfiltercube");
  GstElement *glimagesink1 =
      gst_element_factory_make ("glimagesink", "glimagesink1");

  GstElement *queue2 = gst_element_factory_make ("queue", "queue2");
  GstElement *glimagesink2 =
      gst_element_factory_make ("glimagesink", "glimagesink2");
  ...
  g_signal_connect (identity, "handoff", G_CALLBACK (identityCallback),
      textoverlay);
  g_object_set (G_OBJECT (textoverlay), "font_desc", "Ahafoni CLM Bold 30",
      NULL);
  g_object_set (G_OBJECT (glimagesink0), "client-reshape-callback",
      reshapeCallback, NULL);
  g_object_set (G_OBJECT (glimagesink0), "client-draw-callback", drawCallback,
      NULL);

  /* add elements */
  gst_bin_add_many (GST_BIN (pipeline), videosrc, decodebin, identity,
      textoverlay, tee, queue0, glupload0, glimagesink0, queue1, glupload1,
      glfiltercube, glimagesink1, queue2, glimagesink2, NULL);
  ...
  if (!gst_element_link_many (textoverlay, tee, queue0, glupload0, NULL)) {
    g_warning
        ("Failed to link one or more elements bettween textoverlay and glupload0!\n");
    return -1;
  }

  gboolean link_ok =
      gst_element_link_filtered (glupload0, glimagesink0, cubecaps);
  ...
  if (!gst_element_link_many (tee, queue1, glupload1, glfiltercube,
          glimagesink1, NULL)) {
    g_warning
        ("Failed to link one or more elements bettween tee and glimagesink1!\n");
    return -1;
  }

  if (!gst_element_link_many (tee, queue2, glimagesink2, NULL)) {
    g_warning
        ("Failed to link one or more elements bettween tee and glimagesink2!\n");
    return -1;
  }
